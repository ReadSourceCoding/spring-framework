	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		if (this.propertySources == null) {
			...
			if (this.environment != null) {
				//把 environment 对象封装成 PropertySource 对象加入到
				this.propertySources.addLast(
						//把 environment 对象封装成 PropertySource 对象 MutablePropertySource 中的list
					new PropertySource<Environment>(ENVIRONMENT_PROPERTIES_PROPERTY_SOURCE_NAME, this.environment) {
						@Override
						@Nullable
						//source 对象就是 environment 对象
			...
			try {
				//加载本地配置文件中的属性值包装成properties 对象后，最终包装成 PropertySource 对象
				PropertySource<?> localPropertySource =
						new PropertiesPropertySource(LOCAL_PROPERTIES_PROPERTY_SOURCE_NAME, mergeProperties());
				//加入到 MutablePropertySources 中的list 中
